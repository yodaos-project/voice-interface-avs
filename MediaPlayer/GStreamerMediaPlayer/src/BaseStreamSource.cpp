...
bool BaseStreamSource::init(const AudioFormat* audioFormat) {
    ACSDK_ERROR(LX("BaseStreamSource initzzz"));
    ...
    // TODO: ACSDK-1778: The correct stream type should be set since MediaPlayer
    // checks for seekability using stream type
    // For HTTP media streams, most servers indicate seekability by specifying the Accept-Ranges header. The client that
    // wants to seek will include a Range header to specify the byte offset. For other protocols (such as RTSP), please
    // consult the standard document for that protocol.
    gst_app_src_set_stream_type(appsrc, GST_APP_STREAM_TYPE_SEEKABLE);

    GstCaps* audioCaps = nullptr;

    ACSDK_ERROR(LX("BaseStreamSource init").d("audioFormat", audioFormat));
    if (audioFormat) {
        std::string caps = getCapsString(*audioFormat);
        audioCaps = gst_caps_from_string(caps.c_str());
        ACSDK_ERROR(LX("12121"));
        ACSDK_ERROR(LX("BaseStreamSource audioFormat: " + caps));
        if (!audioCaps) {
...
void BaseStreamSource::clearOnReadDataHandler() {
    ACSDK_ERROR(LX("clearOnReadDataHandlerCalled").d("sourceId", m_sourceId));
    m_sourceRetryCount = 0;
    m_sourceId = 0;
}

void BaseStreamSource::onNeedData(GstElement* pipeline, guint size, gpointer pointer) {
    ACSDK_ERROR(LX("onNeedDataCalled").d("size", size));
...
gboolean BaseStreamSource::handleNeedData() {
    ACSDK_ERROR(LX("handleNeedDataCalled"));
    std::lock_guard<std::mutex> lock(m_callbackIdMutex);
    m_needDataCallbackId = 0;
...
void BaseStreamSource::onEnoughData(GstElement* pipeline, gpointer pointer) {
    ACSDK_ERROR(LX("onEnoughDataCalled"));
    auto source = static_cast<BaseStreamSource*>(pointer);
    std::lock_guard<std::mutex> lock(source->m_callbackIdMutex);
...
gboolean BaseStreamSource::handleEnoughData() {
    ACSDK_ERROR(LX("handleEnoughDataCalled"));
    std::lock_guard<std::mutex> lock(m_callbackIdMutex);
    m_enoughDataCallbackId = 0;
    uninstallOnReadDataHandler();
    return false;
