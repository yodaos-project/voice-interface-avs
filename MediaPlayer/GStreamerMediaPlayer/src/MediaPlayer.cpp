...
bool MediaPlayer::setupPipeline() {
    ...
    ACSDK_ERROR(LX("g_object_set buffering"));        
    ...
    if (configurationRoot) {
        ...
        if (!gst_caps_is_empty(caps)) {
            ...
            ACSDK_ERROR(LX("g_object_set caps"));
...
void MediaPlayer::handleSetUrlSource(
    ...
    bool repeat) {
    ACSDK_ERROR(LX("handleSetSourceForUrlCalled").d("name", RequiresShutdown::name()));
...
void MediaPlayer::sendPlaybackStarted() {
    ACSDK_CRITICAL(LX("sendPlaybackStarted"));
    if (!m_playbackStartedSent) {
